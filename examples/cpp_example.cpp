...
enum Problem { Homography, TwoViewMotion, RigidMotion, Pose6D,Plane3D };
...
int main(int argc, const char* argv[])
{
	...
	for (const std::string& scene : getAvailableTestScenes(Problem::Plane3D))
	{
		printf("Processed scene = %s.\n", scene.c_str());

		testMultiPlaneFitting<100>(
			"data/" + scene + "/" + scene + ".xyz", // The path to the xyz file containing the points
 			"data/" + scene + "/" + scene + "_results.txt", // The path where the results should be saved
			true, // A flag determining if we are loading an oriented point cloud
			250, // The maximum RANSAC iteration number
			0.99, // The RANSAC confidence
			10.0, // The inlier-outlier threshold
			0.0, // The weight of the spatial coherence term
			25, // The cell number along a particular axis in the neighborhood grid
			0.9, // The maximum tanimoto similarity for quick model rejection
			100); // The minimum number of points for a model to be accepted
	}
